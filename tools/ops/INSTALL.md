...
| `digest.py` | `/opt/digest.py` | новая вечерняя сводка: состояние первой строкой, таблица денег, график 30 дней |
| `selfcheck.py` | `/opt/selfcheck.py` | часы биржи по моменту ответа (правка 23.09 против −1021) |
| `pautina.py`, `crowd.py`, `watchdog.sh`, `balance_probe.py` | `/opt/` | **ничего**: в репо лежат копии, байт в байт как на сервере |
| crontab | root | **ничего**: `crontab.txt` в репо = нынешний crontab |
...
Меняется: crontab root становится таким, как в репо. Сейчас он уже такой, шаг не нужен.
