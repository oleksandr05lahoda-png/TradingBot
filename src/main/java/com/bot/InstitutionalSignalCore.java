...
public final class InstitutionalSignalCore {
    ...
    public static final class ActiveSignal {
        ...
        public ActiveSignal(String symbol, TradingCore.Side side, double entry, double confidence, long time) {
            this.symbol = symbol;
            this.side = side;
            this.entry = entry;
    ...
    public boolean allowSignal(DecisionEngineMerged.TradeIdea signal) {
        ...
        int totalSignals = activeSignals.values().stream().mapToInt(List::size).sum();
    ...
    public void registerSignal(DecisionEngineMerged.TradeIdea signal) {
        ...
        activeSignals.computeIfAbsent(signal.symbol, k -> new ArrayList<>()).add(s);
    ...
    public void closeTrade(String symbol, double pnlPercent) {
        ...
        for (ActiveSignal s : list) {
            history.computeIfAbsent(symbol, k -> new ArrayList<>())
                    .add(new ClosedTrade(symbol, pnlPercent, now - s.time));
