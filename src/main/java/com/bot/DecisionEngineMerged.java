...
public final class DecisionEngineMerged {
    ...
    public List<TradeIdea> evaluate(List<String> symbols,
        ...
        for (String symbol : symbols) {
            ...
            if (!valid(c15) || !valid(c1h)) continue;

            MarketState state = detectMarketState(c15);
            HTFBias bias = detectHTFBias(c1h);

            TradeIdea idea = generate(symbol, c15, state, bias, now);
    ...
    private TradeIdea generate(String symbol,
        ...
        switch (state) {
            ...
            case STRONG_TREND -> {
                if (trendStructureLong(c) && bias == HTFBias.BULL) {
                    side = TradingCore.Side.LONG;
                    reason = "Strong trend continuation";
                }
                if (trendStructureShort(c) && bias == HTFBias.BEAR) {
            ...
            case WEAK_TREND -> {
                if (pullbackEntryLong(c, rsi) && bias == HTFBias.BULL) {
                    side = TradingCore.Side.LONG;
                    reason = "Trend pullback entry";
                }
                if (pullbackEntryShort(c, rsi) && bias == HTFBias.BEAR) {
                    side = TradingCore.Side.SHORT;
                    reason = "Trend pullback entry";
                }
            }
            ...
            case CLIMAX -> {
                if (reversalSetup(c, rsi, TradingCore.Side.LONG)) {
                    side = TradingCore.Side.LONG;
                    reason = "Exhaustion reversal";
                }
                if (reversalSetup(c, rsi, TradingCore.Side.SHORT)) {
                    side = TradingCore.Side.SHORT;
                    reason = "Exhaustion reversal";
                }
            }
        ...
        return new TradeIdea(symbol, side, price, stop, take,
                confidence,
                confidence > 0.75 ? SignalGrade.A : SignalGrade.B,
                reason);
    }
    ...
    private boolean trendStructureLong(List<TradingCore.Candle> c) {
        return ema(c,21) > ema(c,50)
                && last(c).close > ema(c,21)
                && higherHighs(c);
    }

    private boolean trendStructureShort(List<TradingCore.Candle> c) {
        return ema(c,21) < ema(c,50)
                && last(c).close < ema(c,21)
                && lowerLows(c);
    }

    private boolean pullbackEntryLong(List<TradingCore.Candle> c, double rsi) {
        return ema(c,21) > ema(c,50)
                && rsi < 45
                && last(c).close > ema(c,50);
    }

    private boolean pullbackEntryShort(List<TradingCore.Candle> c, double rsi) {
        return ema(c,21) < ema(c,50)
                && rsi > 55
                && last(c).close < ema(c,50);
    }

    private boolean reversalSetup(List<TradingCore.Candle> c,
                                  double rsi,
                                  TradingCore.Side side) {
        ...
        if (side == TradingCore.Side.LONG)
            return rsi < 30 && exhaustion;

        return rsi > 70 && exhaustion;
    }
    ...
    private double relativeVolume(List<TradingCore.Candle> c) {
        ...
        double avg = c.subList(n-20,n-1)
                .stream().mapToDouble(cd->cd.volume).average().orElse(0);
