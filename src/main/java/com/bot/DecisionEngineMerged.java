...
public final class DecisionEngineMerged {
    ...
    private TradeIdea generate(String symbol,
        ...
        // [v86.69] FLOW_BREAK — ЧОП-комплемент к TREND (путь к 4/4). В отсутствие 4h-тренда
        // (htfSep<0.4% = точное дополнение TREND) торгуем ПРОБОЙ границы value-area (VAH/VAL),
        // подтверждённый РЕАЛЬНЫМ агрессор-флоу (taker-buy ratio, разблокирован v86.68). Только
        // measure-only тень (FLOW-SHADOW); промоушен в live = зелёный в П2/П4 ≥3 прогона.
        if (stratMode.equals("FLOW_BREAK")) {
            TradeIdea fb = generateFlowBreak(symbol, c15, c1h, cat, now);
            if (fb != null) { csLastSignalTime.put(symbol, now); return fb; }
    ...
    private TradeIdea generateTrendPullback(String symbol,
        ...
        // [v83.4 2026-06-01] ⚡ КЛЮЧЕВОЙ ТЕСТ — ИНВЕРСИЯ НАПРАВЛЕНИЯ (FADE).
        // ДИАГНОЗ из 5 прогонов: WR железно 38.9%, W/L 0.80 — НЕ меняется от
        // фильтра входа(A)/выходов(B)/дистанции TP(C). Это отпечаток СИСТЕМНО
        // НЕВЕРНОГО НАПРАВЛЕНИЯ: breakout-вход проигрывает 61% → FADE выигрывает
        // ~61%. Крипта в рейндже: пробой BB = локальный экстремум → откат.
        // ВАЖНО: инверсия применяется ЗДЕСЬ (после всех направленных фильтров
    ...
    private TradeIdea generateTrendAligned(String symbol,
        ...
        // startup-BT: −0.92 / −1.16 / −0.82 %/сделку, WR 0% в последнем (7 сд, −5.8%).
        // Молодой тренд = свежий кросс с уже разнёсшимися EMA (прошёл htfSep≥0.4%) =
        // импульсный «рывок-откат», непропорционально пила. Режем только <6 закрытых
        // 4h-баров (≈24ч): бакеты 6-12 / 13-24 / 25+ НЕ трогаем (все ≥0). ~5% потока,
        // ВСЕ убыточные → BT-оценка: 153→146 сд, NetPnL +3.4→+9.2, avg +0.022→+0.063%.
        // СТРУКТУРНАЯ ИНВЕРСИЯ провала v86.50 (тот резал ПОБЕДИТЕЛЕЙ по дистанции от
        // EMA); здесь режем подтверждённых лузеров по ВОЗРАСТУ. Тот же generate() путь
        // → backtest==live синхронны. Инструментарий age-таблицы НЕ трогаем (бакет 3-5
        // покажет 0 сд = подтверждение фильтра). ОТКАТ без деплоя: env TA_AGE_MIN=0.
        if (age4h < csEnvDouble("TA_AGE_MIN", 6)) return reject("ta_trend_too_young");

        // [v86.76] AGE-CEILING — режем ЗРЕЛЫЕ/ИСТОЩЁННЫЕ тренды (прямая жалоба юзера:
        // «вхожу на верху после большого движа — как TAO +21% на новости → разворот»).
        // Данные (ВОСПРОИЗВОДИМОЕ окно v86.71, 3 прогона): бакет 25+ баров ТОКСИЧЕН
        // −0.22..−0.23%/сд и это 104/165 сделок (ОСНОВНАЯ МАССА); 13-24 тоже в минусе;
        // единственный +EV — молодые 6-12 (+0.22). Поздний вход = вход в созревший/
        // выдохшийся тренд у вершины. Режем >24 закрытых 4h-баров (тренд старше ~4 дней).
        // Это УБИРАЕТ истощение-входы И срезает главный слив. ВАЖНО: не делает плюс сам по
        // себе (13-24 тоже минус) — для +EV сузить до 12 (TA_AGE_MAX=12, только 6-12, но
        // малая выборка). Прибыль = молодая зона + maker + живая проверка. ОТКАТ: TA_AGE_MAX=999.
        // Воспроизводимый BT покажет эффект чисто (раньше было нельзя — шум ±9×).
        if (age4h > csEnvDouble("TA_AGE_MAX", 24)) return reject("ta_trend_too_old");
        ...
        // [v86.24d REVERTED] The v86.24 continuation-entry branch was pulled: the startup-BT
        // proved it DILUTED the edge (109→170 trades, avg/trade +0.50%→+0.385%, the 70-75 conf
        // bucket flipped to -5.7%, self-validator GO→NO-GO -3.97%). Pullback-only restored =
        // proven baseline. Catching strong no-pullback trends needs proper R&D before it can be
    ...
    private TradeIdea generateFlowBreak(String symbol,
                                        ...
                                        List<com.bot.TradingCore.Candle> c1h,  // HTF (4h)
                                        CoinCategory cat, long now) {
        if (!valid(c15) || !valid(c1h)) return reject("fb_invalid_candles");
        if (CS_SKIP_MEME && cat == CoinCategory.MEME) return reject("fb_skip_meme");
        Long lastSig = csLastSignalTime.get(symbol);
        if (lastSig != null && (now - lastSig) < CS_COOLDOWN_MS) return reject("fb_cooldown");

        // closed-bars only (зеркало v86.15)
        long _nowMs = System.currentTimeMillis();
        if (!c15.isEmpty() && c15.get(c15.size() - 1).closeTime > _nowMs) c15 = c15.subList(0, c15.size() - 1);
        if (!c1h.isEmpty() && c1h.get(c1h.size() - 1).closeTime > _nowMs) c1h = c1h.subList(0, c1h.size() - 1);

        int n = c15.size();
        if (n < 50) return reject("fb_short");
        com.bot.TradingCore.Candle last = c15.get(n - 1);
        double price = last.close;
        if (price <= 0) return reject("fb_bad_price");

        double atr = com.bot.TradingCore.atr(c15, 14);
        if (atr <= 0) return reject("fb_bad_atr");
        double atrPct = atr / price;
        if (atrPct < csEnvDouble("TA_ATR_MIN", 0.004) || atrPct > csEnvDouble("TA_ATR_MAX", 0.06))
        ...
        // ── BTC-гейт (как у TREND) ──
        com.bot.GlobalImpulseController.GlobalContext btc =
                (gicRef != null) ? gicRef.getContext() : null;
        if (btc != null) {
            if (btc.regime == com.bot.GlobalImpulseController.GlobalRegime.BTC_PANIC
             || btc.regime == com.bot.GlobalImpulseController.GlobalRegime.BTC_CRASH) return reject("fb_btc_bad");
            if (wantLong && btc.onlyShort) return reject("fb_btc_only_short");
