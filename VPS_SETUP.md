...
### Telegram: сделки, очередь сканера, «почему вошли» (23.09)

Две необязательные строки в `/opt/tradingbot.env` (после правки — `bash tools/vps-deploy.sh`,
`docker restart` файл не перечитывает):

```
TRADE_NOTIFY=on        # по умолчанию on: строка на каждый вход и выход; off — только их выключает
QUIET_HOURS=23-08      # по Варшаве; ночью строки копятся и приходят одной сводкой 🌙 в 08:00
```

* Каждый вход: `🟢 Вход LONG TIA` — цена, риск, стоп и тейк в процентах от входа.
* Каждый выход: `✅ Тейк` / `🛑 Стоп` / `⏱ По времени` / `✋ Закрыта вручную` / `🔴 Ликвидация` …
  с итогом `≈+$0.51` — это оценка по цене входа и выхода **без комиссий и фандинга**; точный
  нетто — в `/pnl`. Старое английское «Position closed» при включённых строках больше не
  приходит (оно дублировало); предупреждения и критические алерты — как раньше, всегда сразу.
* Тихие часы — по стенным часам Варшавы, и летом, и зимой. Сводка живёт в памяти: рестарт
  бота внутри окна её теряет (журнал сделок и `/pnl` не теряют ничего).
* `/queue` (и кнопка ⏳) — кто ждёт входа и почему не вошёл: «нет места», «стоп шире, чем тянет
  депозит», «пауза после сделки»… плюс закрытия, которые бот повторяет. Читает
  `/opt/tradingbot-data/scanner_view.json`, который сканер пишет в конце каждого прохода. Нет
  файла или он старше 2 ч 15 мин — экран так и скажет.
* `/why ADA` и кнопки монет под `/book` — с какими цифрами сканер открыл позицию и где она
  сейчас. Кнопки монет ничего не закрывают.
